...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    // Your solution here.
	  ...
	  this.run = new DList1();
	  this.run.insertFront(length,0, 0, 0);
	  this.width = width;
	  this.height=height;
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    // Your solution here.
	  int length = width*height;
	  int times = runLengths.length;
	  int check = 0;
	  this.height = height;
	  this.width = width;
	  this.run = new DList1();
	  for(int i=times-1;i>=0;i--){
		  this.run.insertFront(runLengths[i],red[i], green[i], blue[i]);
		  check = check+runLengths[i];
	  }
	  if(check!=length){
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
	  RunIterator start = new RunIterator(this.run.head);
    return start;
  ...
  public PixImage toPixImage() {
	...
	int length = w*h;
	int length2 =this.run.size;
    PixImage ans = new PixImage(w,h);
    int[][] temp = new int[length][3];
    int[][] temp2 = new int[length2][4];
    RunIterator S = this.iterator();
    int count = 0;
    while(S.hasNext()){
    	temp2[count]=S.next();
    	count++;
    ...
    for(int x=0;x<w;x++){
    	for(int y=0;y<h;y++){
    		ans.setPixel(x, y, (short)temp[kk][0], (short)temp[kk][1], (short)temp[kk][2]);
    		kk++;
    	}
    }
    return ans;
  ...
  public String toString() {
    ...
    while(S.hasNext()){
    	temp = S.next();
    	result = result.concat("("+temp[0]+","+temp[1]+","+temp[2]+","+temp[3]+") ");
    }
    return result;
  }
  ...
  public RunLengthEncoding(PixImage image) {
	...
	for (int x=0;x<w;x++){
		for(int y=0;y<h;y++){
			temp[k][0]=image.getRed(x, y);
			temp[k][1]=image.getGreen(x, y);
			temp[k][2]=image.getBlue(x, y);
			
			k = k+1;
		}
	}
	curr[0] = temp[length-1][0];
	curr[1]=temp[length-1][1];
	curr[2]=temp[length-1][2];
	for(int i=length-2;i>=0;i--){
		if(curr[0]==temp[i][0]&&curr[1]==temp[i][1]&&curr[2]==temp[i][2]){
			count =count+1;
		}else{
			
			this.run.insertFront(count, curr[0], curr[1], curr[2]);
  ...
  public void check() {
    // Your solution here.
	  ...
	  int length = w*h;
	  int check=0;
	  check=check+curr.item[0];
	  while(curr.next!=null){
		  if (curr.next.item[1]==curr.item[1]&&curr.next.item[2]==curr.item[2]&&curr.next.item[3]==curr.item[3]){
	  ...
	  if(check!=length){
		  System.out.println("wrong total"+"check:"+check+"  length"+length);
	  }
